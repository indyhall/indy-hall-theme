...
if ( ! function_exists( 'indy_posted_on' ) ) :
	/**
	 * Prints HTML with meta information for the current post-date/time and author.
	 */
	function indy_posted_on() {
		...
		$posted_on = sprintf(
			...
			esc_html_x( 'Posted on %s', 'post date', 'indy' ),
			'<a href="' . esc_url( get_permalink() ) . '" rel="bookmark">' . $time_string . '</a>'
		);

		$byline = sprintf(
			/* translators: the post author */
			esc_html_x( 'by %s', 'post author', 'indy' ),
			'<span class="author vcard"><a class="url fn n" href="' . esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ) . '">' . esc_html( get_the_author() ) . '</a></span>'
		);

		//echo '<span class="posted-on">' . $posted_on . '</span><span class="byline"> ' . $byline . '</span>'; // WPCS: XSS OK.
		echo '<span class="byline"> Posted by ' . $byline . '</span>'; // WPCS: XSS OK.

	}
endif;

if ( ! function_exists( 'indy_entry_footer' ) ) :
	/**
	 * Prints HTML with meta information for the categories, tags and comments.
	 */
	function indy_entry_footer() {
		// Hide category and tag text for pages.
		if ( 'post' === get_post_type() ) {
			/* translators: used between list items, there is a space after the comma */
			$categories_list = get_the_category_list( esc_html__( ', ', 'indy' ) );
			if ( $categories_list && indy_categorized_blog() ) {
				/* translators: the post category */
				printf( '<span class="cat-links">' . esc_html__( 'Posted in %1$s', 'indy' ) . '</span>', $categories_list ); // WPCS: XSS OK.
			}

			/* translators: used between list items, there is a space after the comma */
			$tags_list = get_the_tag_list( '', esc_html__( ', ', 'indy' ) );
			if ( $tags_list ) {
				/* translators: the post tags */
				printf( '<span class="tags-links">' . esc_html__( 'Tagged %1$s', 'indy' ) . '</span>', $tags_list ); // WPCS: XSS OK.
			}
		}

		if ( ! is_single() && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
			echo '<span class="comments-link">';
			comments_popup_link( esc_html__( 'Leave a comment', 'indy' ), esc_html__( '1 Comment', 'indy' ), esc_html__( '% Comments', 'indy' ) );
			echo '</span>';
		}
 ...
 * @return string
 */
function indy_display_svg( $args = array() ) {

	// Make sure $args are an array.
	if ( empty( $args ) ) {
		return esc_html__( 'Please define default parameters in the form of an array.', 'indy' );
	}

	// Define an icon.
	if ( false === array_key_exists( 'icon', $args ) ) {
		return esc_html__( 'Please define an SVG icon filename.', 'indy' );
	}
 ...
 * @return string
 */
function indy_get_the_title( $args = array() ) {
 ...
 * @return string
 */
function indy_get_the_excerpt( $args = array() ) {
 ...
 * @return string
 */
function indy_display_post_image( $size = 'thumbnail' ) {
	...
	$attached_image_url = indy_get_attached_image_url( $size );
 ...
 * @return string
 */
function indy_get_post_image_url( $size = 'thumbnail' ) {
	...
	return indy_get_attached_image_url( $size );
}
 ...
 * @return string
 */
function indy_get_attached_image_url( $size = 'thumbnail' ) {
 ...
 * @return bool
 */
function indy_display_copyright_text() {

	// Grab our customizer settings.
	$copyright_text = get_theme_mod( 'indy_copyright_text' );
 ...
 * @return string
 */
function indy_get_twitter_share_url() {
 ...
 * @return string
 */
function indy_get_facebook_share_url() {
 ...
 * @return string
 */
function indy_get_linkedin_share_url() {
	return add_query_arg(
		array(
 ...
 */
function indy_display_social_network_links() {
	...
	<ul class="social-icons">
		...
		foreach ( $social_networks as $network ) :
			...
			$network_url = get_theme_mod( 'indy_' . $network . '_link' );
			...
			?>
				<li class="social-icon <?php echo esc_attr( $network ); ?>">
					<a href="<?php echo esc_url( $network_url ); ?>">
						...
						<?php
							echo /* translators: the social network name */ sprintf( esc_html_e( 'Link to %s', 'indy' ), ucwords( esc_html( $network ) ) ); // WPCS: XSS OK.
						?>
						</span>
					</a>
				</li><!-- .social-icon -->
 ...
 */
function indy_display_card( $args = array() ) {
	...
	<div class="<?php echo esc_attr( $args['class'] ); ?> card">
		...
		<?php if ( $args['url'] ) : ?>
			<button type="button" class="button button-card" onclick="location.href='<?php echo esc_url( $args['url'] ); ?>'"><?php esc_html_e( 'Read More', 'indy' ); ?></button>
		<?php endif; ?>
 ...
 * @return string
 */
function indy_display_header_button() {
	...
	$button_url  = get_theme_mod( 'indy_header_button_url' );
	$button_text = get_theme_mod( 'indy_header_button_text' );
	?>
	<div class="site-header-action">
		...
		?>
			<a href="<?php echo esc_url( $button_url ); ?>" class="button button-link"><?php echo esc_html( $button_text ?: __( 'More Information', 'indy' ) ); ?></a>
		<?php else : ?>
			<button type="button" class="cta-button" aria-expanded="false" aria-label="<?php esc_html_e( 'Search', 'indy' ); ?>">
				<?php esc_html_e( 'Search', 'indy' ); ?>
			</button>
			<?php get_search_form(); ?>
		<?php endif; ?>
	</div><!-- .header-trigger -->
