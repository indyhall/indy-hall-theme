 ...
 * @return array
 */
function indy_body_classes( $classes ) {
...
add_filter( 'body_class', 'indy_body_classes' );

/**
 * Flush out the transients used in indy_categorized_blog.
 *
 * @return string
 */
function indy_category_transient_flusher() {
	if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) {
		return false;
	}
	// Like, beat it. Dig?
	delete_transient( 'indy_categories' );
}
add_action( 'delete_category', 'indy_category_transient_flusher' );
add_action( 'save_post', 'indy_category_transient_flusher' );
 ...
 * @return string
 */
function indy_excerpt_more( $more ) {
	return sprintf( ' <a class="more-link" href="%1$s">%2$s</a>', get_permalink( get_the_ID() ), esc_html__( 'Read more...', 'indy' ) );
}
add_filter( 'excerpt_more', 'indy_excerpt_more' );
 ...
 * @return array
 */
function indy_custom_mime_types( $mimes ) {
	$mimes['svg']  = 'image/svg+xml';
	$mimes['svgz'] = 'image/svg+xml';
	return $mimes;
}
add_filter( 'upload_mimes', 'indy_custom_mime_types' );
...
add_action( 'wp_head', 'indy_display_customizer_header_scripts', 999 );
add_action( 'wp_footer', 'indy_display_customizer_footer_scripts', 999 );
...
add_shortcode( 'svg', 'indy_display_svg' );
